  ...
  <div class="p-6 space-y-6 max-w-6xl mx-auto">
    <div>
      ...
      <p class="text-gray-600 dark:text-gray-400">
        Overview of your Loke Cards project.
      </p>
    </div>
...
async function refreshFromLocal() {
  ...
  try {
    scenes.value = scopeScenes(normalizeScenes(await getAllScenesLocal()));
  } catch {}
  stats.value = {
...
onMounted(async () => {
  ...
  try {
    ...
    if (Array.isArray(data) && data.length > 0)
      chapters.value = scopeChapters(data);
  } catch (e) {
    /* keep local */
  } finally {
  ...
  try {
    ...
    if (Array.isArray(s) && s.length > 0) scenes.value = scopeScenes(s);
  } catch (e) {
    /* keep local */
  } finally {
  ...
  // Disable live updates to prevent infinite loops
