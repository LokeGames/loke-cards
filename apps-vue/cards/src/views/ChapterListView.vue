      ...
      >
        ...
        >
          ...
          <div class="flex items-center gap-2">
            ...
            <RouterLink
              :to="toNewSceneWithChapter(ch.id)"
              ...
              >New Scene</RouterLink
            >
            <RouterLink
              :to="toEditChapter(ch.id)"
              class="text-sm px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700"
              >Edit</RouterLink
            >
            <button
...
async function refreshFromLocal() {
  try {
    chapters.value = scopeChapters(await getAllChaptersLocal());
  } catch {}
}
...
async function persistOrder() {
  for (const c of chapters.value) {
    ...
    try {
      await api.chapters.update(c.id, payload);
    } catch {
      try {
        await saveChapterLocal(payload);
      } catch {}
    }
  }
}
