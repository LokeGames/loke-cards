...
<div class="flex flex-col gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded">
  <div class="flex gap-2 items-end">
    <div>
      <label class="block text-xs" for="gc-scene-title">Scene Title</label>
      <input id="gc-scene-title" class="rounded border px-2 py-1" bind:value={sceneTitle} placeholder="Title" />
    </div>
    <div>
      <label class="block text-xs" for="gc-scene-id">Scene ID</label>
      <input id="gc-scene-id" class="rounded border px-2 py-1" bind:value={sceneId} placeholder="Optional" />
    </div>
  ...
  <div class="flex gap-2 items-end">
    <div>
      <label class="block text-xs" for="gc-src">Source</label>
      <select id="gc-src" class="rounded border px-2 py-1" bind:value={src} data-testid="gc-src">
    ...
    <div>
      <label class="block text-xs" for="gc-dst">Target</label>
      <select id="gc-dst" class="rounded border px-2 py-1" bind:value={dst} data-testid="gc-dst">
        <option value="">Select…</option>
        {#each scenes as s}
          <option value={s.sceneId}>{s.sceneId}</option>
        {/each}
