...
function normalizeChapter(ch) {
  let src = ch;
  if (ch && ch.data) { try { const parsed = typeof ch.data === 'string' ? JSON.parse(ch.data) : ch.data; src = { ...parsed, id: ch.id || parsed.id }; } catch {} }
...
function normalizeScene(sc) {
  let src = sc;
  if (sc && sc.data) { try { const parsed = typeof sc.data === 'string' ? JSON.parse(sc.data) : sc.data; src = { ...parsed, id: sc.id || parsed.id }; } catch {} }
  ...
  actions: {
    async loadGlobal() {
      ...
      try {
        let chapters = [];
        let scenes = [];
        try { chapters = await api.chapters.getAll(); } catch { chapters = await getAllChaptersLocal(); }
        try { scenes = await api.scenes.getAll(); } catch { scenes = await getAllScenesLocal(); }
        if (!Array.isArray(chapters) || chapters.length === 0) { try { chapters = await getAllChaptersLocal(); } catch {} }
        if (!Array.isArray(scenes) || scenes.length === 0) { try { scenes = await getAllScenesLocal(); } catch {} }
    ...
    async persistScenePosition(sceneId, position) {
      ...
      if (idx >= 0) {
        ...
        try { await saveSceneLocal({ id: sceneId, position, ...this.scenes[idx] }); } catch {}
        try { await api.scenes.update(sceneId, { position }); } catch {}
      }
    },
    async persistChapterPosition(chapterId, position) {
      ...
      if (idx >= 0) {
        ...
        try { await saveChapterLocal({ id: chapterId, position, ...this.chapters[idx] }); } catch {}
        try { await api.chapters.update(chapterId, { position }); } catch {}
      }
    },
    async addChoiceLink(sourceSceneId, targetSceneId) {
      try {
        ...
        try { await api.scenes.update(sourceSceneId, { sceneId: scene.sceneId || scene.id, chapterId: scene.chapterId || scene.chapter, sceneText: scene.sceneText || '', choices: scene.choices || [], stateChanges: scene.stateChanges || [] }); } catch {}
        return true;
      } catch { return false; }
    },
