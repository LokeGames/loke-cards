  ...
  <section aria-labelledby="scenes-heading">
    ...
    <div class="rounded border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900">
      ...
      <ul v-else class="divide-y divide-gray-200 dark:divide-gray-800">
        <li v-for="sc in scenesValid" :key="sc.sceneId || sc.id" class="p-3 flex items-center justify-between">
          <div>
          ...
          <RouterLink :to="toEditScene(sc.sceneId || sc.id)" class="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700">Edit</RouterLink>
        </li>
...
const scenesValid = computed(() => (Array.isArray(props.scenes) ? props.scenes : []).filter(sc => sc && (sc.sceneId || sc.id)))
</script>
